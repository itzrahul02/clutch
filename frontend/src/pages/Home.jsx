...
export default function Home() {
  ...
  useEffect(() => { fetch('/api/games?limit=1').then((response) => response.ok ? response.json() : null).then((payload) => { if (payload?.pagination?.total) setGameCount(payload.pagination.total); }).catch(() => {}); }, []);
  ...
  return <div className="min-h-screen overflow-hidden bg-[#070707] text-white">
    ...
    <main>
      <section className="relative isolate min-h-[760px] overflow-hidden px-6 pb-20 pt-32 md:px-10">
        ...
        <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1.15fr_.85fr] lg:items-center">
          <div className="max-w-3xl">
            ...
            <div className="mt-14 grid max-w-2xl grid-cols-3 border-y border-white/10 py-6">{highlights.map(({ icon, value, label }, index) => <div key={label} className="border-r border-white/10 px-3 last:border-0 sm:px-6"><FontAwesomeIcon icon={icon} className="text-red-400" /><p className="mt-2 text-2xl font-black">{index === 0 ? `${gameCount}+` : value}</p><p className="mt-1 text-xs uppercase tracking-wider text-zinc-400">{label}</p></div>)}</div>
          </div>
      ...
      <section className="border-y border-white/10 bg-zinc-900/40"><div className="mx-auto max-w-7xl px-6 py-24"><p className="section-kicker">Current events</p><h2 className="section-title">Your next game starts here.</h2>{tournaments.length ? <div className="mt-10 grid gap-5 md:grid-cols-3">{tournaments.map((event) => <Link to={`/tournaments/${event.slug}`} key={event._id} className="rounded-xl border border-zinc-800 bg-zinc-950 p-6 transition hover:border-red-500"><p className="text-sm text-red-400">{event.game?.name || 'Clutch event'}</p><h3 className="mt-3 text-xl font-bold">{event.title}</h3><p className="mt-3 text-sm text-zinc-400">{new Date(event.startsAt).toLocaleString()}</p><p className="mt-6 text-sm font-bold">View event <FontAwesomeIcon icon={faArrowRight} className="ml-1" /></p></Link>)}</div> : <div className="mt-10 rounded-xl border border-dashed border-zinc-700 bg-zinc-950/70 p-10 text-center"><p className="text-lg font-bold">New tournaments are landing soon.</p><p className="mt-2 text-zinc-400">Check the tournament hub for the latest announcements.</p><Link to="/tournaments" className="mt-5 inline-block text-red-400">Open tournament hub →</Link></div>}<div className="mt-8 rounded-xl border border-red-900/60 bg-red-950/30 px-5 py-4 text-sm text-red-100"><b>Clutch note:</b> Tournament slots are confirmed only after every player verifies their email. Paid events also require successful payment verification.</div></div></section>
      <section className="mx-auto max-w-7xl px-6 py-24"><div className="grid gap-6 lg:grid-cols-3"><div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-7"><p className="section-kicker">01 · Discover</p><h2 className="mt-4 text-2xl font-black">Pick your game.</h2><p className="mt-3 leading-6 text-zinc-400">Explore supported titles, team-size rules, and registration requirements before you enter.</p><Link to="/games" className="mt-6 inline-block font-bold text-red-400">Browse games →</Link></div><div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-7"><p className="section-kicker">02 · Build</p><h2 className="mt-4 text-2xl font-black">Bring your roster.</h2><p className="mt-3 leading-6 text-zinc-400">Create a team, add every competitor, and keep everyone verified before game day.</p><Link to="/form" className="mt-6 inline-block font-bold text-red-400">Start registration →</Link></div><div className="rounded-2xl border border-red-900 bg-red-950/30 p-7"><p className="section-kicker">03 · Compete</p><h2 className="mt-4 text-2xl font-black">Chase the bracket.</h2><p className="mt-3 leading-6 text-zinc-300">Follow event schedules, brackets, match results, and rankings across Clutch.</p><Link to="/leaderboard" className="mt-6 inline-block font-bold text-red-300">See rankings →</Link></div></div><div className="mt-16 rounded-2xl border border-zinc-800 bg-[linear-gradient(110deg,#18181b,#450a0a)] p-8 text-center sm:p-14"><p className="section-kicker">Ready when you are</p><h2 className="mx-auto mt-4 max-w-2xl text-3xl font-black sm:text-5xl">Your next clutch moment starts with one team.</h2><div className="mt-8 flex flex-wrap justify-center gap-4"><Link to="/games" className="rounded-lg bg-red-600 px-6 py-3 font-bold hover:bg-red-500">Choose a game</Link><Link to="/tournaments" className="rounded-lg border border-zinc-500 px-6 py-3 font-bold hover:border-white">See tournaments</Link></div></div></section>
    </main>
    <SiteFooter />
  </div>;
}
