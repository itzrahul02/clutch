  ...
  return (
    <div>
      ...
      <nav
        ...
        style={{
          backgroundImage: `linear-gradient(rgba(0,0,0,.72),rgba(0,0,0,.72)), url(${background})`,
      ...
      >
        <div className="flex justify-between items-center max-w-7xl mx-auto">
          ...
          <div className="hidden lg:flex gap-5 text-sm">
            ...
            <Link to="/tournaments" className="hover:text-red-600">
              Tournaments
            </Link>
            <Link to="/games" className="hover:text-red-600">
              Games
            </Link>
            <Link to="/leaderboard" className="hover:text-red-600">
              Leaderboard
            </Link>
            <Link to="/community" className="hover:text-red-600">
              Community
            </Link>
            {isAuthenticated ? (
            ...
            {isAuthenticated && (user?.role === 'admin' || user?.role === 'coordinator') ? (
              <Link to="/organizer" className="hover:text-red-600">
                Organizer
              </Link>
            ) : null}
            {!isAuthenticated ? (
          ...
          <a href="https://iiitkota.ac.in/" target="_blank" rel="noreferrer" className="hidden xl:block">
            <img src={collegeLogo} alt="IIIT Kota" className="h-11 w-11 rounded-full border border-red-800" />
          </a>
        ...
        {menuOpen && (
          <div className="lg:hidden absolute top-full left-0 w-full border-t border-white/10 bg-black/95 p-5 flex flex-col items-center gap-4">
            <a href="#" className="hover:text-red-600" onClick={() => setMenuOpen(false)}>
              Home
            </a>
            <Link to="/tournaments" className="hover:text-red-600" onClick={() => setMenuOpen(false)}>
              Tournaments
            </Link>
            <Link to="/games" className="hover:text-red-600" onClick={() => setMenuOpen(false)}>
              Games
            </Link>
            <Link to="/leaderboard" className="hover:text-red-600" onClick={() => setMenuOpen(false)}>
              Leaderboard
            </Link>
            <Link to="/community" className="hover:text-red-600" onClick={() => setMenuOpen(false)}>
              Community
            </Link>
            {isAuthenticated ? (
              <Link to="/dashboard" className="hover:text-red-600" onClick={() => setMenuOpen(false)}>
                Dashboard
              </Link>
            ) : null}
            {isAuthenticated && (user?.role === 'admin' || user?.role === 'coordinator') ? (
              <Link to="/organizer" className="hover:text-red-600" onClick={() => setMenuOpen(false)}>
                Organizer
              </Link>
            ) : null}
            {!isAuthenticated ? (
              <Link to="/login" className="hover:text-red-600" onClick={() => setMenuOpen(false)}>
                Login
              </Link>
            ) : (
              <button
